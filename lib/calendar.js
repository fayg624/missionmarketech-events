...
export function formatGoogleDate(dateStr, timeStr) {
  const cleanTime = timeStr.substring(0, 5) // strips seconds if present
  const dt = new Date(`${dateStr}T${cleanTime}:00`)
  return dt.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z'
}

export function formatISODate(dateStr, timeStr) {
  const cleanTime = timeStr.substring(0, 5)
  return new Date(`${dateStr}T${cleanTime}:00`).toISOString()
}
